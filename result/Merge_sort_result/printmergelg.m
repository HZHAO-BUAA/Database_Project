...
resTL2(:,:)=reconstructedRes(2,:,:);
resVL2(:,:)=reconstructedVars(2,:,:);
...
localvar=resTL2(1:7,2);
resVL2(7:11,2)=resVL2(7:11,1);
errorbar(bufferRange(1:7).*100,localvar,resVL2(1:7,2),'linewidth',1.5)
xlabel('Buffer size M (% file size)'),ylabel('Merging time (milisecond)');
title("Best buffer size given file size = 1 \times 10^6")
set(gca,'FontSize',16);

[Xg,Yg]=meshgrid(bufferRange(1:7).*100,mergetimes);
bs1=resTL2(1:7,:);
surf(Xg,Yg,resTL2(1:7,:)');
hold on
bufferRange(minx(1))
mergetimes(miny(1))
...
title("Merging time distribution given file size = 1 \times 10^6")
set(gca,'FontSize',16);
